...
@section('main')
    ...
    <p style="text-align: center">If systems reach 0% they will be irreparable without first stopping at a ship yard for
        replacement parts.</p>
    @include('components.nav-buttons', ['current' => $division->id])
    <div class="sections">
        @foreach ($systems as $system)
            <div class="systems">
                @include('components.hp', ['system' => $system, 'detail' => true])
                <button class="quick-fix" title="Quick Fix (one action) 1d4 + INT (Engineering)"
                    value="{{ $system->id }}"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img"
                        width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
                        <path fill="currentColor"
                            d="M11 1a4 4 0 0 0-3.896 4.91l-5.051 5.235a1.986 1.986 0 0 0 2.842 2.774l5.007-5.072a4.002 4.002 0 0 0 5.062-4.382a.5.5 0 0 0-.85-.287L12 6.293L9.707 4l2.115-2.115a.5.5 0 0 0-.287-.85A4.032 4.032 0 0 0 10.999 1Z" />
                    </svg></button>
                <button class="focused-repairs" title="Focused Repairs (30 minutes) 1d8 + INT (Engineering)"
                    value="{{ $system->id }}"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img"
                        width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 640 512">
                        <path fill="currentColor"
                ...
                @if (auth()->user()->is_dm && auth()->user()->id == $starship->dm_id)
                    <button class="delete-system" title="Delete System"
                        value="{{ $system->id }}">&NonBreakingSpace;x&NonBreakingSpace;</button>
                    <button class="edit-system" title="Edit System" value="{{ $system->id }}">&#9998;</button>
                @endif
            </div>
        @endforeach
        <div class="systems">
            <div style="max-width: 768px;">
                ...
                @foreach ($systems as $system)
                    {!! strip_tags($system->division_action, ['h1', 'h2', 'h3', 'h4', 'p', 'br', 'i', 'b', 'strong', 'a']) !!}
                @endforeach
            </div>
        </div>
        @if (auth()->user()->is_dm && auth()->user()->id == $starship->dm_id)
            <button id="new-system" value="{{ $division->id }}">+ New System</button>
            <input type="hidden" id="starhip-id" value="{{ $starship->id }}">
        @endif
    </div>
@endsection
