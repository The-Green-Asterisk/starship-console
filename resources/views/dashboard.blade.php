        ...
        <div class="dash sections">
            ...
            <form action="/img/upload" method="POST" name="characterImage" enctype="multipart/form-data">
                ...
                <label for="character-image" id="character-image-wrapper">
                    ...
                    @else
                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" class="character-image"
            ...
            <div class="select-div">
                <select id="character-select">
                    @foreach (auth()->user()->characters as $char)
                        <option value="{{ $char->id }}" {{ $char->is_active == true ? 'selected' : '' }}>
                            {{ $char->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="select-div">
                <select id="starship-select" {{ auth()->user()->characters->count() > 0? '': 'disabled' }}>
                    @if (!$character->starship)
                        <option value="" disabled selected>{{ $character->name }} is not assigned to a starship.
                        </option>
                    @endif
                    @foreach (auth()->user()->starships as $ship)
                        <option value="{{ $ship->id }}" {{ $character->starship?->id == $ship->id ? 'selected' : '' }}>
                            {{ $ship->name }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <div class="division-checkboxes">
                    @foreach ($divisions as $division)
                        <label for="{{ $division->id }}" class="checkbox-label division-checkboxes">
                            <input type="checkbox" id="{{ $division->id }}" name="{{ $division->name }}"
                                value="{{ $division->id }}"
        ...
        <div class="dash sections">
            ...
            <div class="select-div">
                <select id="starship-select"
                    title="{{ auth()->user()->characters->count() > 0? '': 'Please create a character first' }}"
                    {{ auth()->user()->characters->count() > 0? '': 'disabled' }}>
                    <option value="" selected disabled>Starships</option>
                    @if (auth()->user()->starships->count() <= 0)
                        <option value="" disabled>Give your DM your registered email address to be welcomed aboard a
                            new Starship</option>
                    @else
